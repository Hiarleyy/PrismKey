...
core::Result<std::string> HashService::file(const std::filesystem::path& file, const std::string& algorithm) {
    ...
    while (input.read(buffer.data(), static_cast<std::streamsize>(buffer.size())) || input.gcount() > 0) {
        if (EVP_DigestUpdate(context.get(), buffer.data(), static_cast<size_t>(input.gcount())) != 1) {
            return core::Result<std::string>::failure(core::ErrorCode::CryptoError, "Falha ao calcular o hash do arquivo.");
        }
    }
    if (!input.eof()) {
        return core::Result<std::string>::failure(core::ErrorCode::FileNotReadable, "Não foi possível ler o arquivo informado.");
    }

    std::array<unsigned char, EVP_MAX_MD_SIZE> digest{};
    unsigned int digestLength = 0;
    if (EVP_DigestFinal_ex(context.get(), digest.data(), &digestLength) != 1) {
        return core::Result<std::string>::failure(core::ErrorCode::CryptoError, "Falha ao finalizar o cálculo do hash.");
    }
