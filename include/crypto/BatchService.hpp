...
class BatchService {
   public:
    ...
    static std::vector<BatchItemResult> signFiles(const std::vector<std::filesystem::path>& files,
                                                  const std::filesystem::path& privateKey, const std::string& password);
    static std::vector<BatchItemResult> verifyPairs(const std::vector<FilePair>& pairs,
